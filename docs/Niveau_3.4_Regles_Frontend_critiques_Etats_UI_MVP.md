...
> **MVP :** La vérification téléphone est **différée post-MVP**. Tous les utilisateurs peuvent réserver.
...
> **Post-MVP :** États de validation IA (acceptée, incohérences mineures/majeures, fraude) avec pénalités de visibilité.
...
**Disponibilité MVP :**
- Si `status = 'done'` (automatiquement 2h après créneau OU marqué manuellement)
- Tant que `status != 'done'`, feedback indisponible
...
- une UX cohérente
- une base solide pour le futur backend
